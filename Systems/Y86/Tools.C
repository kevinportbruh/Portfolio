...
{
   uint64_t mask = -(byteNum >= 0 && byteNum <= 7);

   return (source >> (8 * byteNum) & mask) & 0xff;
}
...
uint64_t Tools::setBits(uint64_t source, int32_t low, int32_t high)
{
   uint64_t mask = 0;
   if (low >= 0 && high <= 63 && low <= high) {
      mask = ~0ull >> (63 - (high - low));
      source =source | (mask << low);
  }
  return source;
 ...
 * @param int32_t dstlow that is the bit number of the lowest numbered
                         ...
                         int32_t srclow, int32_t dstlow, int32_t length)
{
  ...
  if (srclow < 0 || srclow > 63 || dstlow < 0 || dstlow > 63 || length < 1 || srclow + length > 64 || dstlow + length > 64) {
        return dest;
    }
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
   ...
   uint64_t sign1 = op1 >> 63;
   uint64_t sign2 = op2 >> 63;


   uint64_t signDiff = (op2 - op1) >> 63;


   return ((sign1 ^ sign2) & ((sign1 ^ signDiff) | (sign2 ^ signDiff))) & 1;
}
